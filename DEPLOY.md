# Deploy runbook: recruitingagent.com

Static single-page site served from this repo. Deployed via xCloud push-to-deploy, DNS at the domain registrar for recruitingagent.com.
...
- **Canonical host:** `https://recruitingagent.com` (apex; `www` should redirect to apex)
...
1. In xCloud, add a site/application for **recruitingagent.com** on the target server.
...
At the registrar / DNS host for **recruitingagent.com**, set:

| Type  | Name / Host | Value                        | Notes |
|-------|-------------|------------------------------|-------|
| A     | `@`         | `<xCloud server IP>`         | apex domain |
| A     | `www`       | `<xCloud server IP>`         | or a CNAME to `recruitingagent.com` if the DNS host supports CNAME flattening at the apex |

Remove any conflicting existing A / AAAA / parking records on `@` and `www`. Keep TTL low (e.g. 300s) during cutover.

## 3. SSL

After DNS resolves to the xCloud server (allow up to a few hours for propagation), issue a **Let's Encrypt** certificate in xCloud for both `recruitingagent.com` and `www.recruitingagent.com`. Enable **force HTTPS** and set the `www` to apex redirect.
...
- [ ] `https://recruitingagent.com` loads with a valid SSL padlock
- [ ] `https://www.recruitingagent.com` redirects to the apex
